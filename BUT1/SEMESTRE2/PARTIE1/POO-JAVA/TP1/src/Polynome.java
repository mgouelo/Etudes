...
public class Polynome {
    ...
    public Polynome(double[] coeffs) {
		...
		if (coeffs == null) {
			...
			tab2 = new double[coeffs.length];
			for (int i = 0 ; i < tab2.length ; i++) {
				tab2[i] = coeffs[i];
			}
		}
        this.coefficients = tab2;
    }
    ...
    public boolean isIdentical(Polynome other) {
        if (this == null || other == null) {
            System.err.println("Erreur : le ou les polynomes passés en paramètre sont à null !");
        }
        if (this.coefficients.length >= other.coefficients.length) {
            for (int i = 0 ; i < this.coefficients.length ; i++) {
                if (i >= other.coefficients.length && this.coefficients[i] != 0) {
                    return false;
                } else if ( i < other.coefficients.length && ( other.coefficients[i] != this.coefficients[i] ) ) {
                    return false;
                }
            }
        }
            ...
            for (int i = 0; i < other.coefficients.length; i++) {
                if (i >= this.coefficients.length && other.coefficients[i] != 0) {
                    return false;
                } else if (i < this.coefficients.length && (this.coefficients[i] != other.coefficients[i])) {
                    return false;
                }
            }
        }
